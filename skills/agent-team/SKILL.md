...
6. **Never poll for a verdict from the model.** Push (or post the
   trigger) and start the verdict waiter in the background (Bash
   `run_in_background`) as ONE shell command, then end the turn; the
   harness wakes the session when it exits:
   ```
   git push && node "${CLAUDE_PLUGIN_ROOT:-${AGENT_TEAM_DIR:-$HOME/agent-team}}/bin/wait-for-verdict.mjs" --repo ... --pr ... --head "$(git rev-parse HEAD)"
   ```
   One command, not two: `--since` defaults to the moment the waiter
   itself starts (give or take a second), so a push followed by a
   separately-started waiter reopens the gap an early bot reaction can
   land in and be missed. `CLAUDE_PLUGIN_ROOT` is set inside plugin
   hooks; a session without it (a plain checkout, not installed as a
   plugin) uses its own checkout path instead. Never poll from the
   model, never sleep-loop, never
